...
public class FPSController : NetworkBehaviour {
	...
	void Start () {
        ...
        if (isLocalPlayer)
        {
            ...
            for(int i = 0; i < weapon_FPS.Length; i++)
            {
                weapon_FPS[i].layer = LayerMask.NameToLayer("Player");
            }
        ...
        if (!isLocalPlayer)
        {
            ...
            for (int i = 0; i < weapon_FPS.Length; i++)
            {
                weapon_FPS[i].layer = LayerMask.NameToLayer("Enemy");
            }
    ...
    void Update () {
       ...
       // IF WE ARE NOT THE LOCAL PLAYER
        if (!isLocalPlayer)
        { 
            //IF THIS IF STATEMENT IS TRUE WE EXECUTE THE return CODE
            //ALL BELOW NOT EXECUTE
            return;
        }
        PlayerMovement();
        SelectWeapon();
    ...
    void PlayerMovement() {
        ...
        if (is_Grounded)
        {
            // HERE WE ARE GONNA CALL CROUCH SPRINT
            PlayerCrouchAndSprinting();
            moveDirection = new Vector3(inputX * inputModifyFactor, -antiBumpFactor, inputY * inputModifyFactor);

            moveDirection = transform.TransformDirection(moveDirection) * speed;

            // HERE WE GONNA CALL JUMP
            PlayerJump();
        }
   ...
   IEnumerator MoveCameraCrouch()
    {
        charController.height = is_Crouching ? defaultControllerHeight / 1.5f : defaultControllerHeight;
